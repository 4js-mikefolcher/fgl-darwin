...
- `util.Regex` does not exist â correct class is `util.Regexp`
- `getKeys()`, `getAsObject()`, `getAsArray()` do not exist on
  util.JSONObject â use `name(i)`, `getType(key)`, `get(key)`
- `ELSE IF` does not exist in BDL â use nested `IF` inside `ELSE`
- `$variable` is for static SQL, `?` is for dynamic SQL â agents
  reverse these
- `ON CHANGE` is only valid inside INPUT sub-dialogs, not at the
  outer DIALOG level
- `DEFINE` must be at top of FUNCTION/MAIN â not inside IF/FOR/CASE
